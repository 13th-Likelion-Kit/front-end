...
const a1 = 10;
const b1 = 20;
console.log(`${a1} + ${b1} = ${a1 + b1}`); // 10 + 20 = 30입니다.
...
let newList = nameList.filter(name => name !== "장은서"); // "장은서"를 제거하는 filter 메소드 사용
...
list2.map(mapping => {
  console.log(`${mapping.title}, ${mapping.content}`);
}); // 제목과 내용을 순서대로 출력합니다.
...
const result = list3.filter(mapping => mapping.like >= 100); 
...
const result2 = list4.reduce((acc, mapping) => acc + mapping.like, 0); 
...
const result3 = list5.some(mapping => mapping.like >= 100); 
...
const result4 = list6.every(mapping => mapping.like >= 100); 
console.log(result4); // like가 100 이상인 것만 있는지 확인합니다.          


// 수고하셨습니다. github에 올려주세요.~~
